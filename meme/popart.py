...
POP_PALETTES = [
    None,
    [(255, 60, 60), (255, 220, 60), (60, 200, 60), (60, 180, 255), (180, 60, 255), (60, 60, 60)],
    [(60, 200, 255), (60, 60, 255), (200, 60, 255), (255, 60, 200), (60, 200, 200), (200, 200, 60)],
    [(255, 255, 255), (0, 0, 0), (255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0)],
    [(255, 200, 0), (255, 100, 0), (200, 0, 0), (100, 0, 0), (50, 50, 50), (200, 200, 200)],
]
...
def _make_multipanel(img_rgba, num_colors, edge_width, halftone_size, panels):
    """从单张 RGBA 图像生成多联画 canvas。"""
    side = min(img_rgba.size)
    cx, cy = img_rgba.size[0] // 2, img_rgba.size[1] // 2
    square = img_rgba.crop((cx - side // 2, cy - side // 2,
                            cx + side // 2, cy + side // 2))
...
def popart_image(input_path, output_path,
    ...
    if panels > 1:
        if not is_gif(input_path):
            canvas = _make_multipanel(src.convert("RGBA"), num_colors, edge_width, halftone_size, panels)
            canvas.save(output_path, "PNG")
        else:
            frames, durations = unfold_frames(src)
            processed = [_make_multipanel(f.convert("RGBA"), num_colors, edge_width, halftone_size, panels) for f in frames]
